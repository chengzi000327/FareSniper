...
describe('OpenMicDeck', () => {
  ...
  test('explains the end-to-end architecture and evaluation harness', async () => {
    ...
    expect(screen.getByText(/登录凭证 JWT → user_id/)).toBeInTheDocument()
    expect(screen.getByText(/Redis 取回已确认的航线、日期等条件/)).toBeInTheDocument()
    expect(screen.getByText(/FlyAI CLI · 实时查询/)).toBeInTheDocument()
    expect(screen.getByText(/收齐所有可用来源/)).toBeInTheDocument()
    expect(screen.getByText(/遍历全部候选/)).toBeInTheDocument()
    expect(screen.getByText(/确认身份 · 接上对话 · 真实查询/)).toBeInTheDocument()
    ...
    fireEvent.click(screen.getByRole('button', { name: '第 9 页：二期迭代' }))
    expect(await screen.findByRole('heading', { name: '从产品闭环，走向可复用的 Agent 能力' })).toBeInTheDocument()
    ...
    expect(screen.getByText(/由 Hermes \+ Cron 调用/)).toBeInTheDocument()
    expect(screen.getByText(/Hermes · Cron · 飞书 · 微信/)).toBeInTheDocument()
    expect(screen.getByText('下一步 · 沉淀或开源为 Skill / API，接入用户自己的 Hermes')).toBeInTheDocument()
  })
})

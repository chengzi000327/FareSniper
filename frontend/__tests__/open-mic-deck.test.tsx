...
describe('OpenMicDeck', () => {
  ...
  test('explains the end-to-end architecture and evaluation harness', async () => {
    ...
    expect(screen.getByText(/收敛可执行任务/)).toBeInTheDocument()
    expect(screen.getByText(/FlightOffer → ResponseFacts/)).toBeInTheDocument()
    expect(screen.getByText(/FastAPI \+ Redis \+ PostgreSQL/)).toBeInTheDocument()

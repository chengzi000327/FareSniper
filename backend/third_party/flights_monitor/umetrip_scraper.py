...
航旅纵横机票爬虫
域名: www.umetrip.com
API: /mightySite/flight/search 触发 /flight/search/list 接口
"""
import json
import logging
...
(function() {
    window.__umetripFlightResponses = [];

    const _matchUrl = function(url) {
        return url && (
            url.indexOf('umetrip.com') !== -1 ||
            url.indexOf('airmiles.cn') !== -1
        ) && (
            ...
            if (_matchUrl(url)) {
                response.clone().text().then(function(body) {
                    try {
                        ...
                        if (d && (d.data || d.flights || d.flightList || d.result)) {
                            window.__umetripFlightResponses.push(body);
                        }
                    } catch(e) {}
                });
            }
        ...
        if (_matchUrl(this._url)) {
            xhr.addEventListener('load', function() {
                try {
                    ...
                    if (d && (d.data || d.flights || d.flightList || d.result)) {
                        window.__umetripFlightResponses.push(xhr.responseText);
                    }
                } catch(e) {}
            });
        }
...
class UmetripScraper(BaseScraper):
    ...
    def build_search_url(self, dcity: str, acity: str, date_str: str) -> str:
        return (
            f"https://www.umetrip.com/mightySite/flight/search.html"
            f"?type=0&depCity={dcity.upper()}&arrCity={acity.upper()}"
            f"&depDate={date_str}&cabin=Y"
        )

    def wait_js_check(self) -> str:
        return "return !!(window.__umetripFlightResponses && window.__umetripFlightResponses.length > 0)"

    def extract_raw(self, driver) -> List[str]:
        raw = driver.execute_script(
            "var r = window.__umetripFlightResponses || [];"
            "window.__umetripFlightResponses = [];"
    ...
    def parse_raw(
        ...
        for item in flight_list:
            try:
                ...
                dep_airport = first.get("depAirportName") or first.get("depAirport", "")
                arr_airport = last.get("arrAirportName") or last.get("arrAirport", "")
                duration = _to_minutes(
                    item.get("duration") or item.get("flyMinutes")
                    or sum(s.get("duration", 0) for s in segs)
                )
                transfer_time = _to_minutes(item.get("stopTime", 0))
                transfer_cities = [
                    s.get("arrAirportName", "") for s in segs[:-1]
                    if s.get("arrAirportName")
                ]

                price, discount_rate = _extract_umetrip_price(item)
                if price <= 0:
                    continue
...
def _extract_umetrip_price(item: dict):
    """提取航旅纵横最低经济舱价格，返回 (price, discount_rate)"""
    price_list = (
        item.get("priceList") or item.get("prices")
        or item.get("cabins") or item.get("fareList") or []
    )
    best_price = 0
    best_rate = 0.0
    for p in price_list:
        cabin = str(p.get("cabin") or p.get("cabinClass") or p.get("subClass") or "Y")

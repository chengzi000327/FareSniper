...
**范围：** 国内携程机票价格采集、Railway 快照接入、现有价格卡片展示，以及中国大陆、香港、澳门和台湾完整商业运输机场城市目录
...
- 城市解析覆盖中国大陆全部民航运输机场，并覆盖香港、澳门和台湾全部有商业客运能力的机场城市。
- 支持城市中文名、常用简称、机场中文名、IATA 机场码、ICAO 机场码和 IATA 多机场城市码。
...
- 不把军用机场、通用航空机场、直升机场或仅在建且未投入运输服务的机场作为可订票目的地。
...
- 中国大陆全部颁证民用运输机场。以民航局最新年度运输机场统计名单为覆盖基线。
- 香港、澳门全部有商业客运能力的机场。
- 台湾全部有商业客运能力的机场，包括本岛和离岛机场。

目录按机场保存，不把“一个城市”错误简化为“一个机场代码”。北京、上海、成都等多机场城市保留一个城市实体和多个机场实体；城市搜索使用 Provider 支持的多机场城市码，用户明确输入机场名或机场码时保留该机场约束。

目录包含运输机场，即使当前没有目标航线库存也保留城市解析能力。实际搜索是否有票由 Provider 返回，不在城市解析阶段虚构可售航线。

### 5.2 数据来源与版本

仓库提交生成后的 `backend/data/china_airports.json`，运行时不依赖第三方在线机场 API。

数据更新脚本使用以下来源：

1. 民航局年度全国民用运输机场统计名单，作为中国大陆机场覆盖和中文名称的权威基线。
2. OurAirports 每日开放数据，补充 IATA、ICAO、经纬度、时区和机场状态。
3. 香港、澳门和台湾机场管理机构公开资料，用于校验当地商业客运机场和中文名称。
4. 仓库内受审查的别名覆盖文件，处理自治州、地区、旗、县级市、旧机场名、少数民族名称和多机场城市码。

生成文件记录数据版本、生成时间和来源摘要。更新脚本不得在应用启动或用户请求时联网。

### 5.3 目录模型

每个城市实体至少包含：

- 稳定 `city_id`、中文标准名、省级区域和 `region_group`（`mainland`、`hong_kong`、`macau`、`taiwan`）。
- 中文别名、常用简称和可选拼音别名。
- Provider 使用的城市码和多机场城市码。
- 一个或多个机场实体。

每个机场实体至少包含：

- 中文机场名、IATA 三字码、ICAO 四字码。
- 是否为运输机场、是否有商业客运能力、当前状态。
- 经纬度、时区和数据来源标记。

### 5.4 单一解析入口

新增统一 `AirportCatalog`，提供：

- `resolve_city(text)`：城市名、别名或城市码解析到城市实体。
- `resolve_airport(text)`：机场名、IATA 或 ICAO 解析到机场实体。
- `resolve_location(text)`：自动区分城市和具体机场，保留用户约束。
- `city_to_provider_code(city, provider)`：按 FlyAI、携程、VariFlight 或 SerpAPI 输出正确代码。
- `code_to_city(code)`：机场码和城市码反查标准中文城市。

`backend/utils/airport_codes.py`、`backend/third_party/flights_monitor/shared.py`、推荐服务和 VariFlight 中的重复映射改为调用该目录，不再各自维护城市字典。无法解析或存在真实歧义时进入补槽流程，不把原始字符串当成有效机场码继续请求 Provider。

### 5.5 采集调度约束

完整城市目录只扩大“用户可以搜索的范围”，不生成所有城市两两组合任务。携程任务仍仅来自：

- 用户近期搜索。
- 已启用价格监控的航线。
- 明确配置的热门航线。

港澳台航线在目录中可解析，但按跨境航线调度，不能误分类为中国大陆国内航线。Provider 是否支持该航线由统一路由规则判断。
...
- 目录总量满足版本元数据声明，且不存在重复 IATA、ICAO 或稳定 ID。
- 北京、上海、成都等多机场城市正确解析城市码和具体机场码。
- 大陆、香港、澳门和台湾代表性城市均能通过中文、机场名和代码解析。
- 军用、通航、直升机场和未投入运输服务机场不会成为可订票城市。
- 港澳台航线不会被分类为中国大陆国内航线。
...
- 验证意图解析、Provider 参数、快照查询和预订跳转使用同一个机场目录结果。
...
- 中国大陆、香港、澳门和台湾全部目标运输机场城市存在于版本化目录中，目录覆盖数与来源元数据一致。
- 任意目录城市都能进入搜索和按需采集流程，不受热门城市硬编码限制。
- 一市多机场、具体机场约束和港澳台跨境分类正确。

## 15. 实施顺序

1. 建立版本化中国机场目录、更新脚本和统一解析 API，迁移重复城市映射。
2. 修复携程依赖与错误模型，删除真实结果中的虚构字段。
3. 建立 Railway 任务、租约、快照和 Collector 内部 API。
4. 建立 Mac Collector CLI、持久 Profile、心跳和安全上传。
5. 增加 `launchd` 安装与首次登录流程。
6. 接入 Ctrip Snapshot Provider 和可空价格字段。
7. 建立基于最终 `deals` 的确定性事实渲染和 AI 文本一致性校验。
8. 调整现有卡片状态、平台展示价排序和携程跳转。
9. 补充 LangSmith、集成测试和真实国内航线验证。
